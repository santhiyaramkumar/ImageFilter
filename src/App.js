...
function App() {
  ...
  const styles = { display: 'inline', width: '20%', height: 80, float: 'left', padding: 5,  marginBottom: 10, marginRight: 10 }
  ...
  return (
    <div>
        ...
        <input style={{ width:"30%", marginTop:20,marginBottom:20 }} type="text" onChange={(e) => handleSearchImage(e)} />
        <div >
          {searchimage.map((value) => {

            return (
              <div style={styles}>
                <img style={{ width: 40, marginBottom:10 }} alt="image1" src={value.url} />
                <div>{value.title}</div>
              </div>
            )
          })}
